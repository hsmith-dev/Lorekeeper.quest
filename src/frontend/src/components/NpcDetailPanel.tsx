...
import { useUpdateNpc, useNpcEntries, useGenerateNpcBio } from "../hooks/useNpcs";
import { formatDate } from "../utils/formatDate";
import type { NpcEntry, NpcRole, NpcStatus } from "../types";

const ROLE_OPTIONS: { value: NpcRole; label: string }[] = [
  { value: "npc", label: "NPC" },
  { value: "pc", label: "Player Character" },
  { value: "monster", label: "Monster / Boss" },
...
export function NpcDetailPanel({ npc, campaignId, onClose }: Props) {
  const [name, setName] = useState("");
  const [role, setRole] = useState<NpcRole>("npc");
  const [status, setStatus] = useState<NpcStatus>("unknown");
  const [description, setDescription] = useState("");
  const [notes, setNotes] = useState("");
  const [saved, setSaved] = useState(false);

  const update = useUpdateNpc(campaignId);
  const generate = useGenerateNpcBio(campaignId);
  const { data: entries, isLoading: entriesLoading } = useNpcEntries(npc?.id ?? null);
  ...
  const handleSave = () => {
    update.mutate(
      { id: npc.id, data: { name, role, status, description: description || undefined, notes: notes || undefined } },
      { onSuccess: () => { setSaved(true); setTimeout(() => setSaved(false), 2000); } }
    );
  };

  const handleGenerate = () => {
    generate.mutate(npc.id, {
  ...
  return (
    <SlideOver open={!!npc} onClose={onClose} title={npc.name}>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              ...
              <select value={role} onChange={(e) => setRole(e.target.value as NpcRole)} className={inputClass}>
                {ROLE_OPTIONS.map((o) => (
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select value={status} onChange={(e) => setStatus(e.target.value as NpcStatus)} className={inputClass}>
        ...
        <div className="border-t border-border pt-4 flex flex-col gap-3">
          ...
          {!entriesLoading && !entries?.length && (
            <p className="text-xs text-muted-foreground italic">
              No journal entries mention "{npc.name}" yet.
            </p>
          )}
